...
class ModelLoader:
    ...
    def load(self) -> Tuple[object, object]:
        ...
        if not adapter_dir.exists():
            raise RuntimeError(
                f"Adapter path '{adapter_dir}' does not exist. "
                "Train adapters first or switch to Base mode."
            )
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to load adapters from '{adapter_dir}'. "
                "Ensure adapters were trained against the same base MODEL_NAME."
            ) from exc
